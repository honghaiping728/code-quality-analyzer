...
| `cq-web` | 启动模块 | Spring Boot 启动入口、28 个 REST 接口、黑白简约前端控制台 | ✅ 已实现 |
...
浏览器访问 **http://localhost:8080/** ，在「概览」页输入服务器上可访问的目录路径即可发起扫描，
随后在「问题列表」「问题详情」「报告与趋势」「规则配置」中查看结果。
...
| 接口 | 说明 |
| --- | --- |
| `POST /api/scan/start` | 提交扫描任务，异步执行并返回任务 ID |
